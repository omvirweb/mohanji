...
class Datatables
{
	...
	function __construct(array $config = array())
	{
		...
		if (count($config) > 0){
                    if(isset($config['database'])){
                        $this->ci->load->database($config['database'],true);
	...
	private function _get_datatables_query()
	{
		...
		if(!empty($this->custom_likes)){
			foreach($this->custom_likes as $custom_like){
				$column_name = $custom_like['column_name'];
				$column_value = $custom_like['column_value'];
				$this->ci->db->like($column_name,$column_value);
			}
		}
...
//		echo $this->ci->db->last_query();
	...
	public function count_all()
	{
        ...
        if($this->ci->uri->segment(1)!="department_attendance" && $this->ci->uri->segment(2)!="present_datatable") {
            if(!empty($this->joins)){
                foreach($this->joins as $join){
                    if(isset($join['join_table']) &&  isset($join['join_by']) && $join['join_table'] != '' && $join['join_by'] != ''){
                        $join_table = $join['join_table'];
