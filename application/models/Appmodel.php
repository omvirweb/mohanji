...
class AppModel extends CI_Model
{
    ...
    function login($username,$pass){
        $user_sql_query = "SELECT * FROM user_master WHERE login_username = BINARY '".$username."' AND user_password = BINARY '".$pass."' AND user_type != ".USER_TYPE_WORKER." LIMIT 1 ";
        $query = $this->db->query($user_sql_query);
        if($query->num_rows() > 0){
...
//        echo "<pre>"; print_r($array); exit;
    ...
    {
        $status = 0;
        $user_roles = $this->session->userdata(PACKAGE_FOLDER_NAME . 'user_roles');
