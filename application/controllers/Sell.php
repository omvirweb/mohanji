...
//        echo '<pre>'; print_r($select2_data); exit;
            ...
            if($this->applib->have_access_role(SELL_PURCHASE_MODULE_ID,"edit")) {
                ...
                if(!empty($sell_data_items)){
                    foreach($sell_data_items as $lot_item){
                        ...
                        $type_name = $this->crud->get_column_value_by_id('sell_type', 'type_name2', array('sell_type_id' => $lot_item->type));
                        $sell_items->type_name = $type_name;
                        ...
                        $sell_items->gold_silver_rate = (isset($lot_item->gold_silver_rate) && !empty($lot_item->gold_silver_rate)) ? $lot_item->gold_silver_rate : 0;
    ...
    function sell_list($param1 = '') { 
        if($this->applib->have_access_role(SELL_PURCHASE_MODULE_ID,"view")) {
            if(in_array($param1,array("sell","purchase")) && $this->sell_purchase_difference) {
                ...
                if($sell_purchase == "sell") {
                    $page_label = "Sell";
                    $entry_page_url = base_url("sell/add/sell");
                } else {
                    $page_label = "Purchase";
                    $entry_page_url = base_url("sell/add/purchase");
                }
            } else {
                $sell_purchase = "sell_purchase";
                $view = $param1;
                $page_label = "Sell/Purchase";
                $entry_page_url = base_url("sell/add");
            }
    ...
    function get_account_old_balance($account_id){
        ...
        if(!empty($account_id)){
            ...
            if(!empty($account_data)){
                ...
                $data['account_mobile'] = (!empty($account_data[0]->account_mobile)) ? $account_data[0]->account_mobile : '-';
                $data['account_state'] = $account_data[0]->account_state;

                //$sql="select sum(fine) as fine from sell_items where account_id=".$account_id;
                $this->db->select_sum('si.fine','fine');
                ...
                $this->db->where('c.category_group_id',1);

                $get_total_gold_fine_purchase = $this->db->get();
                ...
                $this->db->where('a.account_id',$account_id);
                $this->db->where('si.type',2);

                $this->db->where('c.category_group_id',1);

                $get_total_gold_fine_sale = $this->db->get();

                // echo $this->db->last_query();
                // exit;


                $this->db->select_sum('si.metal_fine','fine');
                $this->db->from('metal_payment_receipt si');
                $this->db->join('sell s', 's.sell_id = si.sell_id', 'left');
                $this->db->join('category c', 'c.category_id = si.metal_category_id', 'left');
                $this->db->join('item_master item', 'item.item_id = si.metal_item_id', 'left');
                $this->db->join('account a', 'a.account_id = s.account_id', 'left');


                $this->db->where('a.account_id',$account_id);
                $this->db->where('si.metal_payment_receipt',1);

                $this->db->where('c.category_group_id',1);

                $get_metal_total_gold_fine_purchase = $this->db->get();
                ...
                $this->db->where('a.account_id',$account_id);
                $this->db->where('si.metal_payment_receipt',2);

                $this->db->where('c.category_group_id',1);

                $get_metal_total_gold_fine_sale = $this->db->get();


                // echo $this->db->last_query();
                // exit;

                $this->db->select_sum('si.gold_weight','fine');
                $this->db->from('gold_bhav si');
                $this->db->where('si.gold_sale_purchase',1);
                $this->db->join('sell s', 's.sell_id = si.sell_id', 'left');
                $this->db->join('account a', 'a.account_id = s.account_id', 'left');


                $this->db->where('a.account_id',$account_id);
                $get_total_gold_bhav_sale = $this->db->get();
                ...
                $this->db->select_sum('si.gold_weight','fine');
                $this->db->from('gold_bhav si');
                $this->db->where('si.gold_sale_purchase',2);
                $this->db->join('sell s', 's.sell_id = si.sell_id', 'left');
                $this->db->join('account a', 'a.account_id = s.account_id', 'left');


                $this->db->where('a.account_id',$account_id);
                $get_total_gold_bhav_purchase = $this->db->get();

                // echo $this->db->last_query();
                // exit;


                $this->db->select_sum('mhm_detail.fine','fine');
                ...
                $this->db->where('mhm_detail.type_id',1);

                $this->db->where('a.account_id',$account_id);

                $get_total_mhmifw_purchase = $this->db->get();
                ...
                $this->db->where('mhm_detail.type_id',2);

                $this->db->where('a.account_id',$account_id);

                $get_total_mhmifw_purchase2 = $this->db->get();
                ...
                $this->db->where('mhm_detail.type_id',3);

                $this->db->where('a.account_id',$account_id);

                $get_total_mhmifw_sale = $this->db->get();

                // echo $this->db->last_query();
                // exit;
                

                $this->db->select_sum('ird.fine','fine');
                ...
                $get_total_mfi_purchase = $this->db->get();
                ...
                $this->db->where('ird.type_id',2);

                $this->db->where('a.account_id',$account_id);

                $get_total_mfi_sale = $this->db->get();

                // echo $this->db->last_query();
                // exit;
                

                $this->db->select_sum('ir.ir_diffrence','fine');
                ...
                $get_total_irkw = $this->db->get();

                // echo $this->db->last_query();
                // exit;


                $this->db->select_sum('tr.transfer_gold','fine');
                ...
                $get_naam_jama_sale= $this->db->get();
                ...
                $get_naam_jama_purchase= $this->db->get();
                ...
                $get_naam_jama_purchase2= $this->db->get();

                // echo $this->db->last_query();
                // exit;


                $get_naam_jama_purchase_new=$get_naam_jama_purchase2->result()[0]->fine-$get_naam_jama_purchase->result()[0]->fine;


                $this->db->select_sum('whd.fine_adjusted','fine');
                ...
                $this->db->where('whd.type_id',1);

                $get_whd_sale= $this->db->get();
                ...
                $this->db->where('whd.type_id',2);

                $get_whd_purchase= $this->db->get();
                ...
                // $where = "`mhm_detail`.`type_id` = '".MANUFACTURE_HM_TYPE_ISSUE_FINISH_WORK_ID."' || `mhm_detail`.`type_id` = '".MANUFACTURE_HM_TYPE_ISSUE_SCRAP_ID."'";
                ...
                $get_mhd_purchase= $this->db->get();

                // echo $this->db->last_query();
                // echo "get_whd_purchase =".$get_mhd_purchase->result()[0]->fine;
                // exit;


                // echo "get_total_mhmifw_purchase :".+$get_total_mhmifw_purchase->result()[0]->fine." sale :".$get_total_mhmifw_sale->result()[0]->fine;
                // exit;

                // var_dump($get_total_gold_fine);exit;

                // $data['gold_fine'] = $account_data[0]->gold_fine;

                //code by daya and babu bhai
                // $data['gold_fine'] = round($get_total_gold_fine_purchase->result()[0]->fine-$get_total_gold_fine_sale->result()[0]->fine+$get_metal_total_gold_fine_purchase->result()[0]->fine-$get_metal_total_gold_fine_sale->result()[0]->fine+$get_total_gold_bhav_purchase->result()[0]->fine-$get_total_gold_bhav_sale->result()[0]->fine,3);

                $opening_data = $this->get_opening_customer_ledger('2019-04-01', $account_id, 0, '');

                // echo "<pre>";print_r($opening_data);exit;
                // echo 'opening ='.$opening_data[0]->gold_fine;exit;

                // $data['gold_fine'] = round($get_total_gold_fine_purchase->result()[0]->fine-$get_total_gold_fine_sale->result()[0]->fine+$get_metal_total_gold_fine_purchase->result()[0]->fine-$get_metal_total_gold_fine_sale->result()[0]->fine+$get_total_gold_bhav_purchase->result()[0]->fine-$get_total_gold_bhav_sale->result()[0]->fine+$get_total_mhmifw_purchase->result()[0]->fine+$get_total_mhmifw_purchase2->result()[0]->fine-$get_total_mhmifw_sale->result()[0]->fine+$get_total_irkw->result()[0]->fine+$get_total_mfi_purchase->result()[0]->fine-$get_total_mfi_sale->result()[0]->fine+$get_naam_jama_purchase->result()[0]->fine-$get_naam_jama_sale->result()[0]->fine+$opening_data[0]->gold_fine,3);
                // $data['gold_fine'] = round($get_total_gold_fine_purchase->result()[0]->fine-$get_total_gold_fine_sale->result()[0]->fine+$get_metal_total_gold_fine_purchase->result()[0]->fine-$get_metal_total_gold_fine_sale->result()[0]->fine+$get_total_gold_bhav_purchase->result()[0]->fine-$get_total_gold_bhav_sale->result()[0]->fine+$get_total_mhmifw_purchase->result()[0]->fine+$get_total_mhmifw_purchase2->result()[0]->fine-$get_total_mhmifw_sale->result()[0]->fine+$get_total_irkw->result()[0]->fine+$get_total_mfi_purchase->result()[0]->fine-$get_total_mfi_sale->result()[0]->fine-$get_naam_jama_purchase_new-$get_naam_jama_sale->result()[0]->fine+$opening_data[0]->gold_fine,3);


                $data['gold_fine'] = round($get_total_gold_fine_purchase->result()[0]->fine-$get_total_gold_fine_sale->result()[0]->fine+$get_metal_total_gold_fine_purchase->result()[0]->fine-$get_metal_total_gold_fine_sale->result()[0]->fine+$get_total_gold_bhav_purchase->result()[0]->fine-$get_total_gold_bhav_sale->result()[0]->fine+$get_total_mhmifw_purchase->result()[0]->fine+$get_total_mhmifw_purchase2->result()[0]->fine-$get_total_mhmifw_sale->result()[0]->fine+$get_total_irkw->result()[0]->fine+$get_total_mfi_purchase->result()[0]->fine-$get_total_mfi_sale->result()[0]->fine-$get_naam_jama_purchase_new-$get_naam_jama_sale->result()[0]->fine+$opening_data[0]->gold_fine-$get_whd_purchase->result()[0]->fine+$get_whd_sale->result()[0]->fine-$get_mhd_purchase->result()[0]->fine,3);
...
//                    $old_sell_adjust_cr_ids = implode(',', $old_sell_adjust_cr_id_arr);
            ...
            $insert_arr['entry_through'] = ENTRY_THROUGH_SELL_PURCHASE_DEFAULT;
...
//                    }
    ...
    function sell_datatable() {
        ...
        $config['wheres'][] = array('column_name' => 's.entry_through', 'column_value' => ENTRY_THROUGH_SELL_PURCHASE_DEFAULT);
...
//        echo '<pre>'; print_r($list); exit;
        ...
        foreach ($list as $sell) {
            ...
            if($sell->account_id != ADJUST_EXPENSE_ACCOUNT_ID){
                ...
                if($sell->audit_status != AUDIT_STATUS_AUDITED){
                    if(isset($post_data['sell_purchase']) && $post_data['sell_purchase'] == "sell") {
                        $action .= '<a href="' . base_url("sell/add/sell/" . $sell->sell_id) . '"><span class="edit_button glyphicon glyphicon-edit data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';                        
                    } elseif(isset($post_data['sell_purchase']) && $post_data['sell_purchase'] == "purchase") {
                        $action .= '<a href="' . base_url("sell/add/purchase/" . $sell->sell_id) . '"><span class="edit_button glyphicon glyphicon-edit data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';

                    } else {
                        $action .= '<a href="' . base_url("sell/add/" . $sell->sell_id) . '"><span class="edit_button glyphicon glyphicon-edit data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';
                    }
                    ...
                    if(isset($post_data['sell_purchase']) && $post_data['sell_purchase'] == "sell") {
                        ...
                        $action .= '<a href="' . base_url("sell/add/" . $sell->sell_id) . '"><span class="edit_button glyphicon glyphicon-eye-open data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';
                    }
            ...
            $action .= '<a href="' . base_url("sell/sell_print_1/" . $sell->sell_id) . '" target="_blank" title="Sell/Purchase Print" alt="Sell/Purchase Print"><span class="glyphicon glyphicon-print" style="color : #bc3ca3">&nbsp</a>';
            if(PACKAGE_FOR != 'manek') {
            ...
            if(isset($post_data['account_id']) && !empty($post_data['account_id'])){
                $action .= '<input type="checkbox" class="checkbox_for_change_status" data-sell_id="' . $sell->sell_id . '" >';
            }
            $row[] = $action;
    ...
    function audit_status_sell_for_multiple() {
        $return = array();

        if(isset($_POST['audit_status_sell_ids']) && !empty($_POST['audit_status_sell_ids']) && isset($_POST['audit_status']) && !empty($_POST['audit_status'])){
            foreach($_POST['audit_status_sell_ids'] as $audit_status_sell_id){
                $result = $this->crud->update('sell', array('audit_status' => $_POST['audit_status']), array('sell_id' => $audit_status_sell_id));
                $this->crud->update('payment_receipt', array('audit_status' => $_POST['audit_status']), array('sell_id' => $audit_status_sell_id));
                $return['success'] = "Changed";
            }
        } else {
            $return['errro'] = "Error";
                ...
                foreach($sell_data_items as $sell_data_item){
                    ...
                    if($sell_items->type != SELL_TYPE_SELL_ID){
                        ...
                        if(PACKAGE_FOR == 'manek') {
                            ...
                            $sell_items->amount = ZERO_VALUE - $sell_items->amount;
                        }
                    } else {
                    ...
                    $sell_items->gold_silver_rate = (isset($sell_data_item->gold_silver_rate) && !empty($sell_data_item->gold_silver_rate)) ? $sell_data_item->gold_silver_rate : 0;
                    ...
                    if(PACKAGE_FOR == 'manek') {
                        ...
                        $charges_details .= " WHERE `sell_item_charges_details`.`sell_item_id` = " . $sell_data_item->sell_item_id;
                        $charges_details = $this->crud->getFromSQLArray($charges_details);
                        $sell_items->charges_details = (!empty($charges_details)) ? $charges_details[0]['charges_details'] : '';
                    }
...
//        print_r($data); exit;
        ...
        if($isimage == 'isimage'){
            ...
            if(PACKAGE_FOR == 'manek') {
                $html = $this->load->view('sell/sell_print_manek', $data, true);
            } else {
                $html = $this->load->view('sell/sell_print', $data, true);
            }
            if(PACKAGE_FOR == 'mohanji' || PACKAGE_FOR == 'manek') {
    ...
    function sell_print_1($sell_id = '', $isimage = ''){
        $this->load->library('numbertowords');
        $user_data=$this->session->userdata(PACKAGE_FOLDER_NAME.'is_logged_in');
        $data = array();
...
//                echo '<pre>'; print_r($metal_payment_receipt); exit;
    ...
    //    print_r($data); exit;
        ...
        $data['company_address'] = $this->crud->get_column_value_by_id('settings', 'settings_value', array('settings_key' => 'company_address'));
        $data['ask_discount_in_sell_purchase'] = $this->crud->get_column_value_by_id('settings', 'settings_value', array('settings_key' => 'ask_discount_in_sell_purchase'));
        $html = $this->load->view('sell/sell_print_1', $data, true);
        $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'A5']);
        $mpdf->AddPage(
            'P', //orientation
            '', //type
