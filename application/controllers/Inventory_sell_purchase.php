...
class Inventory_sell_purchase extends CI_Controller {
    ...
    function add($param1 = '',$param2 = '',$param3 = '') {
        if(in_array($param1,array("sell","purchase")) && $this->sell_purchase_difference) {
            ...
            if($sell_purchase == "sell") {
                ...
                $list_page_url = base_url("inventory_sell_purchase/splist/sell");
            } else {
                $page_label = "Purchase";
                $page_shortcut = "[CTRL + F2]";
                $list_page_url = base_url("inventory_sell_purchase/splist/purchase");
            }
        } else {
            ...
            $page_shortcut = "[CTRL + F1]";
            $list_page_url = base_url("inventory_sell_purchase/splist");
        }
        $data = array();
...
//                    $data['credit_limit'] = $this->crud->get_id_by_val('account', 'credit_limit', 'account_id', $account_id);
                ...
                set_page('inventory_sell_purchase/add', $data);
            } else {
    ...
    function splist($param1 = '') { 
        if($this->applib->have_access_role(SELL_PURCHASE_MODULE_ID,"view")) {
            if(in_array($param1,array("sell","purchase")) && $this->sell_purchase_difference) {
                ...
                if($sell_purchase == "sell") {
                    $page_label = "Sell";
                    $entry_page_url = base_url("inventory_sell_purchase/add/sell");
                } else {
                    $page_label = "Purchase";
                    $entry_page_url = base_url("inventory_sell_purchase/add/purchase");
                }
            } else {
                $sell_purchase = "sell_purchase";
                $view = $param1;
                $page_label = "Sell/Purchase";
                $entry_page_url = base_url("inventory_sell_purchase/add");
            }
            ...
            set_page('inventory_sell_purchase/splist', $data);
        } else {
            $this->session->set_flashdata('error_message', 'You have not permission to access this page.');
            redirect("/");
        }
        ...
        foreach ($list as $sell) {
            ...
            if($sell->account_id != ADJUST_EXPENSE_ACCOUNT_ID){
                ...
                if($sell->audit_status != AUDIT_STATUS_AUDITED){
                    if(isset($post_data['sell_purchase']) && $post_data['sell_purchase'] == "sell") {
                        $action .= '<a href="' . base_url("inventory_sell_purchase/add/sell/" . $sell->sell_id) . '"><span class="edit_button glyphicon glyphicon-edit data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';                        
                    } elseif(isset($post_data['sell_purchase']) && $post_data['sell_purchase'] == "purchase") {
                        $action .= '<a href="' . base_url("inventory_sell_purchase/add/purchase/" . $sell->sell_id) . '"><span class="edit_button glyphicon glyphicon-edit data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';

                    } else {
                        $action .= '<a href="' . base_url("inventory_sell_purchase/add/" . $sell->sell_id) . '"><span class="edit_button glyphicon glyphicon-edit data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';
                    }
                    ...
                    if(isset($post_data['sell_purchase']) && $post_data['sell_purchase'] == "sell") {
                        $action .= '<a href="' . base_url("inventory_sell_purchase/add/sell/" . $sell->sell_id) . '"><span class="edit_button glyphicon glyphicon-eye-open data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';
                    
                    } elseif(isset($post_data['sell_purchase']) && $post_data['sell_purchase'] == "purchase") {
                        $action .= '<a href="' . base_url("inventory_sell_purchase/add/purchase/" . $sell->sell_id) . '"><span class="edit_button glyphicon glyphicon-eye-open data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';
                    
                    } else {
                        $action .= '<a href="' . base_url("inventory_sell_purchase/add/" . $sell->sell_id) . '"><span class="edit_button glyphicon glyphicon-eye-open data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';
                    }
                    
                }
            }
            $action .= '<a href="' . base_url("inventory_sell_purchase/sell_print/" . $sell->sell_id) . '" target="_blank" title="Sell/Purchase Print" alt="Sell/Purchase Print"><span class="glyphicon glyphicon-print" style="color : #419bf4">&nbsp</a>';
            if(PACKAGE_FOR != 'manek') {
                $action .= '<a href="' . base_url("inventory_sell_purchase/sell_print/" . $sell->sell_id . '/isimage/') . '" target="_blank" title="Sell/Purchase Print as a Image" alt="Sell/Purchase Print as a Image"><span class="glyphicon glyphicon-picture" style="color : #419bf4">&nbsp</a>';
            }
            $audit_status = '';
            if($sell->audit_status == AUDIT_STATUS_AUDITED){
                $audit_status = 'A';
...
//            echo $this->db->last_query();
        ...
        foreach ($list as $sell_detail) {
            ...
            if (isset($post_data['sell_id']) && !empty($post_data['sell_id'])) {} else{
                if($role_edit){
                    $action .= '<a href="' . base_url("inventory_sell_purchase/add/" . $sell_detail->sell_id) . '"><span class="edit_button glyphicon glyphicon-edit data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';
                }
...
//                if(!empty($sell_item->grwt)){
                    ...
                    $sell_lineitem['huid'] = (isset($sell_item->huid)) ? $sell_item->huid : '';
...
//            $purchase_delete_array = array_merge($purchase_items, $metal_items, $stock_transfer_items);
    ...
    function sell_item_list(){
        if($this->applib->have_access_role(SELL_PURCHASE_MODULE_ID,"sell/purchase item list")) {
            $data = array();
            set_page('inventory_sell_purchase/sell_item', $data);
        } else {
            $this->session->set_flashdata('error_message', 'You have not permission to access this page.');
            redirect("/");
        }
    ...
    function item_datatable(){
        ...
        foreach ($list as $item_detail) {
            ...
            $row[] = '<a href="'. base_url("inventory_sell_purchase/sell_item_list_from_item/".$item_detail->item_id.'/'.$from_date.'/'.$to_date).'" target="_blank">'.$item_detail->item_name.'</a>';
    ...
    function sell_item_list_from_item($item_id='',$from_date='',$to_date=''){
        if($this->applib->have_access_role(SELL_PURCHASE_MODULE_ID,"sell/purchase item list")) {
            ...
            $touch = $this->crud->get_all_records('carat', 'purity', 'ASC');
            $data['touch'] = $touch;
            set_page('inventory_sell_purchase/sell_item_list', $data);
        } else {
            $this->session->set_flashdata('error_message', 'You have not permission to access this page.');
            redirect("/");
        }
