...
//        echo '<pre>'.$this->db->last_query(); exit;
        ...
        if($account_group_id == DEPARTMENT_GROUP){
            ...
            $sell_items_data = array_merge($sell_items_data1, $sell_with_gst_amount_data);
...
//            echo '<pre>'; print_r($list_row); exit;
            ...
            $outstanding_data_arr[$account_id]['account_name'] = '<a href="'.base_url().'reports/customer_ledger?account_id='.$list_row->account_id.'" target="_blank">'.$list_row->account_name.'</a>';
...
//            $checked = $cashbook->is_payment_received == 1 && empty($cashbook->sell_id) ? 'checked' : '';
    ...
    function get_all_department_cash() {
        $data = array();
        $amount = $this->crud->getFromSQL('SELECT SUM(amount) as amount FROM account WHERE account_group_id = "' . DEPARTMENT_GROUP . '"');
        $amount = !empty($amount) ? $amount[0] : 0;
        $data['amount'] = $amount->amount;
        print json_encode($data);
        exit;
...
//            $data['account'] = $account;
            ...
            set_page('reports/interest',$data);
        } else {
            $this->session->set_flashdata('error_message', 'You have not permission to access this page.');
            redirect("/");
        }
...
//                    $interest_accounts[] = $acc_row;
            ...
            set_page('reports/generate_interest', $data);
        } else {
            $this->session->set_flashdata('error_message', 'You have not permission to access this page.');
            redirect("/");
        }
...
//        print_r($interest_data_arr);
    ...
    function customer_ledger() {
        if($this->applib->have_access_role(ACCOUNT_MODULE_ID,"customer ledger")) {
            ...
            $data['ledger_print_in_page_a5'] = $this->crud->get_column_value_by_id('settings', 'settings_value', array('settings_key' => 'ledger_print_in_page_a5'));
            $data['from_date'] = date("d-m-Y");
            $data['default_from_financial_start_year'] = $this->crud->get_column_value_by_id('settings', 'settings_value', array('settings_key' => 'default_from_financial_start_year'));
            if($data['default_from_financial_start_year'] == '1'){
                $data['from_date'] = $this->applib->get_financial_start_date_by_date($data['from_date']);
            }
            set_page('reports/customer_ledger', $data);
        } else {
            $this->session->set_flashdata('error_message', 'You have not permission to access this page.');
            redirect("/");
        }
...
//            $from_zero_arr = $this->applib->multi_array_search($reverse_data, array('14' => '0', '15' => '0', '16' => '0'));
    ...
    function balance_sheet() {
        if ($this->applib->have_access_role(REPORT_MODULE_ID, "view") && $this->applib->have_access_role(BALANCE_SHEET_MODULE_ID, 'view')) {
            ...
            $data['gold_rate'] = $this->crud->get_column_value_by_id('settings', 'settings_value', array('settings_key' => 'gold_rate'));
            $data['silver_rate']= $this->crud->get_column_value_by_id('settings', 'settings_value', array('settings_key' => 'silver_rate'));
            set_page('reports/balance_sheet', $data);
        } else {
            $this->session->set_flashdata('error_message', 'You have not permission to access this page.');
            redirect("/");
        }
    ...
    function balance_sheet_datatable() {
        ...
        foreach ($list as $key => $list_row) {
            ...
            $outstanding_data_arr[$account_id]['account_name'] = '<a href="'.base_url().'reports/outstanding?account_id='.$list_row->account_id.'" target="_blank">'.$list_row->account_name.'</a>';
            $outstanding_data_arr[$account_id]['account_mobile'] = $list_row->account_mobile;
...
//        $capital += number_format((float) $silver_net_amount_for_capital, 2, '.', '');
        ...
        if($post_data['balance_sheet_format'] == "group") {
            ...
            foreach ($account_groups_data as $key => $account_groups_row) {
                $account_groups_name_arr[$account_groups_row['account_group_id']] = '<a href="'.base_url().'reports/outstanding?account_group_id='.$account_groups_row['account_group_id'].'" target="_blank">'.$account_groups_row['account_group_name'].'</a>';
            }
