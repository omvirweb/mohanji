...
<div class="content-wrapper" id="body-content">
    <form class="form-horizontal" method="post" id="save_sell_purchase" novalidate enctype="multipart/form-data">
        <?php if (isset($order_data->order_id) && !empty($order_data->order_id)) { ?>
            <input type="hidden" name="order_id" id="order_id" value="<?= $order_data->order_id ?>"/>
        <?php } ?>
        ...
        <section class="content-header">
            <h1>
                ...
                <?php if (isset($order_data->order_id) && !empty($order_data->order_id)) { } else { if($isAdd) { $btn_disable = null; } else { $btn_disable = 'disabled';} } ?> 
                <button type="submit" name="saveform" id="saveform" class="btn btn-primary pull-right module_save_btn btn-sm" ><?= isset($order_data->order_id) ? 'Update' : 'Save' ?> [ Ctrl +S ]</button>
                <a href="<?= base_url('order/order_list') ?>" class="btn btn-primary pull-right btn-sm" style="margin: 5px;" <?php echo isset($order_data->order_id) ? '' : $btn_disable;?>>Order List</a>
                            ...
                            <div class="box-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-1">
                                        ...
                                        <input type="text" name="sell_no" id="sell_no" class="form-control" readonly value="<?= (isset($order_data->order_no) && !empty($order_data->order_no)) ? $order_data->order_no : ''; ?>"><br />
                                    </div>
                                    <?php } ?>
                                    <div class="col-md-3">
                                    ...
                                    <?php } else { ?>
                                        <input type="hidden" name="process_id" id="process_id" value="<?php echo (isset($order_data->process_id) && !empty ($order_data->process_id)) ? $order_data->process_id : $this->session->userdata(PACKAGE_FOLDER_NAME.'is_logged_in')['default_department_id']; ?>" >
                                    <?php } ?>
                                    <div class="col-md-1">
                                        <label for="date">Date <span class="required-sign">&nbsp;*</span></label>
                                        <input type="text" name="order_date" id="datepicker2" class="form-control input-datepicker" value="<?php if (isset($order_data->order_date) && !empty($order_data->order_date)) { echo date('d-m-Y', strtotime($order_data->order_date)); } else { echo date('d-m-Y'); } ?>" style="padding: 5px;"><br />
                                    </div>
                                    <?php if($remark_2 == '1') { ?>
                                        <div class="col-md-3">
                                            <label for="order_remark">Remark</label>
                                            <input type="text" name="order_remark" id="order_remark" class="form-control" value="<?php if (isset($order_data->order_remark) && !empty($order_data->order_remark)) { echo $order_data->order_remark; } else { echo '';} ?>"><br />
                                        </div>
                                    <?php } ?>
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <select name="delivery_type" id="delivery_type" class="form-control select2" >
                                            <option value="1" <?= (isset($order_data->delivery_type) && !empty($order_data->delivery_type)) ? ($order_data->delivery_type == '1') ? 'Selected' : '' : 'Selected';?> >Delivered</option>
                                            <option value="2" <?= (isset($order_data->delivery_type) && !empty($order_data->delivery_type)) ? ($order_data->delivery_type == '2') ? 'Selected' : '' : '';?>>Not Delivered</option>
                                        </select>
                                        <div class="clearfix"></div><br />
                                    </div>
                                    <?php } ?>
                                    ...
                                    <div class="line_item_form item_fields_div" style="height: 160px;">
                                        ...
                                        <input type="hidden" name="line_items_data[order_item_delete]" id="order_item_delete" value="allow" />
                                        <?php if (isset($order_item) && !empty($order_item)) { ?>
                                        ...
                                        <div class="col-md-12">
                                            <!--<label>&nbsp;</label>-->
                                            <input type="button" id="add_lineitem" class="btn btn-info btn-sm add_lineitem pull-right" value="Add Item" style="margin:7px; <?php if (isset($order_data->order_id) && !empty($order_data->order_id)) { ?> margin-left: 10px; <?php } else { ?> margin-left: 10px; <?php } ?>"/>
                                            <?php if (isset($order_data->order_id) && !empty($order_data->order_id)) { ?>
                                                <div class="created_updated_info pull-right" style="margin-left: 10px;">
                                                   Created by : <?php echo isset($order_data ->created_by_name) ? $order_data ->created_by_name :'' ; ?>
                                                   @ <?php echo isset($order_data ->created_at) ? date('d-m-Y h:i A',strtotime($order_data ->created_at)) :'' ; ?><br/>
                                                   Updated by : <?php echo isset($order_data ->updated_by_name) ? $order_data ->updated_by_name :'' ;?>
                                                   @ <?php echo isset($order_data ->updated_at) ? date('d-m-Y h:i A',strtotime($order_data ->updated_at)) : '' ;?>
                                                </div>
                                            <?php } ?>
                                            ...
                                            <tbody id="pop_up_total_list">
                                                <?php if($ask_discount_in_sell_purchase == '1') { ?>
                                                    <tr>
                                                        ...
                                                        <th width="7%" class="text-right"></th>
                                                        <th width="6%" class="text-right"><input type="text" name="discount_amount" id="discount_amount" class="form-control input-sm" value="<?php if (isset($order_data->discount_amount) && !empty($order_data->discount_amount)) { echo $order_data->discount_amount; } else { echo '0';} ?>"></th>
                                                        <?php if($c_r_amount_separate == '1') { ?>
                                                            <th width="5%"></th>
                                                            <th width="5%"></th>
                                                        <?php } ?>
...
<?php if (isset($order_item)) { ?>
    var li_lineitem_objectdata = [<?php echo $order_item; ?>];
...
<?php } ?>
    ...
    $(document).ready(function () {
        ...
        <?php } elseif (isset($order_data->account_id) && !empty ($order_data->account_id)) { ?>
            get_bill_balance(<?php echo $order_data->account_id; ?>);
            setSelect2Value($("#account_id"), "<?= base_url('app/set_account_name_with_number_val_by_id/' . $order_data->account_id) ?>");
            initAjaxSelect2($("#transfer_account_id"), "<?= base_url('app/set_account_name_with_number_val_by_id/' . $order_data->account_id) ?>");
        <?php } else { ?>
            setSelect2Value($("#account_id"), "<?= base_url('app/set_account_name_with_number_val_by_id/' . CASE_CUSTOMER_ACCOUNT_ID) ?>");
        <?php } ?>
        <?php if (isset($order_data->account_id) && !empty ($order_data->account_id)) { ?>  
            initAjaxSelect2($("#transfer_account_id"), "<?= base_url('app/account_name_from_main_party_select2_source/' . $order_data->account_id.'/1') ?>");
        <?php } else { ?>
        ...
        <?php if($department_2 == '1') { ?>
            ...
            <?php } elseif (isset($order_data->process_id) && !empty ($order_data->process_id)) { ?>
                setSelect2Value($("#process_id"), "<?= base_url('app/set_process_master_select2_val_by_id/' . $order_data->process_id) ?>");
            <?php } else { ?>
                    ...
                    success: function (response) {
                        ...
                        if (json.stock_method == '2') {
                            ...
                            if(sell_type_id == <?php echo SELL_TYPE_SELL_ID; ?>){
                                ...
                                var sell_id = '';
                                <?php if (isset($order_data->order_id) && !empty($order_data->order_id)) { ?>
                                    sell_id = '<?php echo $order_data->order_id; ?>';
                                <?php } ?>
                                $.ajax({
...
//            console.log(pts_grwt + ' ' + pe_stock_grwt);
        ...
        <?php if (isset($order_data->discount_amount) && !empty($order_data->discount_amount)) { ?>
...
//            });
        ...
        $(document).on('click', '#order_to_sell_button', function () {
            ...
            $.each($("input.selected_index:checked"), function() {
                ...
                ots_lineitem_objectdata[selected_index].order_item_delete = 'allow';
...
//            console.log(lineitem_objectdata);
            ...
            $.each($("input.pts_selected_index:checked"), function() {
                ...
                var pts_selected_index = $(this).data('pts_selected_index');
                pts_lineitem_objectdata[pts_selected_index].order_item_delete = 'allow';
...
//            lineitem_objectdata = pts_selected_index_lineitems;
        ...
        <?php if(!isset($order_data->audit_status) || (isset($order_data->audit_status) && $order_data->audit_status != AUDIT_STATUS_AUDITED)){ ?>
            <?php if(!isset($order_data->account_id) || (isset($order_data->account_id) && $order_data->account_id != ADJUST_EXPENSE_ACCOUNT_ID)){ ?>
...
//            console.log(clickbtn['selector']); return false;
            ...
            if (lineitem_objectdata == '' && pay_rec_objectdata == '' && metal_objectdata == '' && gold_objectdata == '' && silver_objectdata == '' && transfer_objectdata == '' && ad_charges_objectdata == '' && adjust_cr_objectdata == '') {
                show_notify("Please Add Order Item.", false);
                return false;
            }
            var account_id = $('#account_id').val();
            ...
            var href_url = '<?= base_url('order/save_order') ?>';
            if(is_grater == 1){
                if (confirm('Credit Limit : ' + credit_limit)) {
                    ...
                    <?php if (isset($order_data->order_id) && !empty($order_data->order_id)) { ?>
                        if (($("#deleted_sell_item_id").length > 0)){
                            var sell_id = '<?php echo $order_data->order_id;?>';
                            $.ajax({
                    ...
                    $.each(lineitem_objectdata, function (li_index, li_value) {
                        if(li_value.purchase_sell_item_id == value.purchase_sell_item_id && li_value.stock_type == value.stock_type){
                            if(row_added == 0){ 
                                ...
                                '</td>' +
                                '<td>' + value.account_name + ' ['+ value.order_date +']</td>' +
                                '<td>' + value.item_name + '</td>' +
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    <?php if(isset($order_data) && !empty($order_data)){ ?>
                        var sell_data_total_gold_fine = <?php echo $order_data->total_gold_fine; ?>;
                        var sell_data_total_silver_fine = <?php echo $order_data->total_silver_fine; ?>;
                        var sell_data_total_amount = <?php echo $order_data->total_amount; ?>;
                        var sell_data_total_c_amount = <?php echo $order_data->total_c_amount; ?>;
                        var sell_data_total_r_amount = <?php echo $order_data->total_r_amount; ?>;
...
//                amount = zero_value - parseFloat(amount);
            ...
            if(value.order_item_delete == 'allow'){
...
//        setSelect2Value($("#sell_type_id"), "<?= base_url('app/set_sell_type_select2_val_by_id/') ?>" + value.type);
        ...
        if(value.order_item_delete == 'not_allow'){
        ...
        $("#order_item_delete").val(value.order_item_delete);
...
//            console.log(input.files);
    ...
    function save_form(postData, href_url){
        ...
        postData.append('order_amount', bill_amount);
