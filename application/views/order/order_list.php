                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="box-body table-responsive">
                                        ...
                                        <div class="select_all_div hide">
                                            <label><input type="checkbox" id="select_all_for_change_status" /> Select All</label>
                                            <a href="javascript:void(0);" class="audit_status_button" data-audit_status_sell_id="" data-audit_status="" style="margin: 8px;"> Change Status</a>
                                        </div>
                                        <table id="lot_master_table" class="table row-border table-bordered table-striped" style="width:100%">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Department</th>
                                                    <th>Order No</th>
                                                    <th  width="80px" class="text-nowrap">Order Date</th>
                                                    <th>Remark</th>
...
//            "paging": false,
            ...
            "ajax": {
                "url": "<?php echo site_url('order/order_datatable') ?>",
                "type": "POST",
                "data": function (d) {
                    <?php if(isset($view_not) && !empty($view_not)){ ?>
                    <?php } else { ?>
        ...
        item_table = $('#item_table').DataTable({
            ...
            "paging": false,
            "ajax": {
                "url": "<?php echo site_url('order/order_item_datatable') ?>",
                "type": "POST",
                "data": function (d) {
        ...
        $(document).on('click', '#search', function (){
            $("#ajax-loader").show();
            $('.select_all_div').addClass('hide');
            var account_id = $('#account_id').val();
            if(account_id != '' && account_id != null){
                $('.select_all_div').removeClass('hide');
            }
            table.draw();
        });
        ...
        $(document).on("click", "#change_audit_status", function () {
            ...
            if(audit_status_sell_id != '' && audit_status != ''){
                ...
                var sell_id_arr = [];
                $.each($(".checkbox_for_change_status:checked"), function(){
                    var sell_id = $(this).data('sell_id');
                    sell_id_arr.push(sell_id);
                });
                if (sell_id_arr.length != 0) {
                    $.ajax({
                        url: "<?php echo site_url('sell/audit_status_sell_for_multiple') ?>",
                        type: "POST",
                        async: false,
                        data: {audit_status_sell_ids : sell_id_arr, audit_status : audit_status},
                        success: function (response) {
                            var json = $.parseJSON(response);
                            $("#ajax-loader").hide();
                            if (json['success'] == 'Changed') {
                                ...
                                show_notify('Audit Status Changed Successfully!', true);
                                $('#audit_status_modal').modal('hide');
                                $('#select_all_for_change_status').prop('checked', false); // Unchecks it
                            } else {
                                show_notify('Somthing was Wrong!', false);
                            }
                        }
        ...
        $(document).on('click', '#select_all_for_change_status', function () {
            if($("#select_all_for_change_status").is(':checked')){
                    $('.checkbox_for_change_status').prop('checked', true);
            } else {
                    $('.checkbox_for_change_status').prop('checked', false);
            }
        });
    });
