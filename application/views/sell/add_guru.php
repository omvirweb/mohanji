                            ...
                            <div class="box-body">
                                <div class="row">
                                    ...
                                    <div class="line_item_form item_fields_div" style="height: 160px;">
                                        ...
                                        <?php if($charges_2 == '1') { ?>
                                            <div class="col-md-1">
                                                <label for="pcs_input">Pcs</label>
                                                <input type="text" name="line_items_data[pcs_no]" id="pcs_input" class="form-control num_only">
                                            </div>
                                            <div class="col-md-1">
                                                <label for="pcs_charges">Pcs Charges</label>
                                                <input type="text" name="line_items_data[pcs_charges]" id="pcs_charges" class="form-control num_only">
                                            </div>
                                            <div class="col-md-2">
...
<?php } ?>
    ...
    $(document).ready(function () {
        ...
        $(document).ready(function(){
            $("#pcs_input,#pcs_charges").keyup(function(){
                var pcs = $('#pcs_input').val();
                var pcscharge = $('#pcs_charges').val();
                if(pcs && pcscharge){
                    $('#charges_amt').val(parseFloat(pcs)*parseFloat(pcscharge));
                }else{
                    $('#charges_amt').val(0);
                }
            });
...
//        initAjaxSelect2($("#sell_type_id"), "<?= base_url('app/sell_type_select2_source/') ?>" + sell_purchase );
        if(sell_purchase == 'sell'){
            ...
            $('#sell_type_id').append($("<option>Sel</option>").attr("value", "<?php echo SELL_TYPE_SELL_ID ?>")); 
        } else if(sell_purchase == 'purchase'){
            $('#sell_type_id').append($("<option>--</option>").attr("value", "")); 
            $('#sell_type_id').append($("<option>Exch</option>").attr("value", "<?php echo SELL_TYPE_EXCHANGE_ID ?>")); 
            $('#sell_type_id').append($("<option>Pur</option>").attr("value", "<?php echo SELL_TYPE_PURCHASE_ID ?>")); 
        } else {
            $('#sell_type_id').append($("<option>--</option>").attr("value", "")); 
            $('#sell_type_id').append($("<option>Exch</option>").attr("value", "<?php echo SELL_TYPE_EXCHANGE_ID ?>")); 
            $('#sell_type_id').append($("<option>Pur</option>").attr("value", "<?php echo SELL_TYPE_PURCHASE_ID ?>")); 
            $('#sell_type_id').append($("<option>Sel</option>").attr("value", "<?php echo SELL_TYPE_SELL_ID ?>")); 
            $('#sell_type_id').append($("<option>Issue</option>").attr("value", "<?php echo SELL_TYPE_ISSUE_ID ?>"));
            $('#sell_type_id').append($("<option>Rec</option>").attr("value", "<?php echo SELL_TYPE_REC_ID ?>"));
        }
...
//                var charges_amt_per = parseFloat(total_sell_item_charges_details_amount) / parseFloat(lineitem_objectdata[sell_lineitem_charges_details_for_index].gross_amount) * 100;
        ...
        $(document).on('change', '#sell_type_id', function () {
            ...
            } else if (sell_type_id == '4') {
                $('.field_required').attr('hidden', 'hidden');
                $(".item_fields_div").css("background-color", "#83d3e7");
                $('.touch_select').hide();
                $('.touch_input').show();
                $("#tunch_textbox").attr('disabled', 'disabled');
            } else if (sell_type_id == '5') {
                $('.field_required').attr('hidden', 'hidden');
                $(".item_fields_div").css("background-color", "#a3c3cb");
                $('.touch_select').hide();
                $('.touch_input').show();
                $("#tunch_textbox").attr('disabled', 'disabled');
            } else {
...
//            console.log(clickbtn['selector']); return false;
    ...
    function display_lineitem_html(lineitem_objectdata) {
        ...
        $.each(lineitem_objectdata, function (index, value) {
            ...
            if(value.type_name == 'Sel' || value.type_name == 'Issue'){
            } else {
