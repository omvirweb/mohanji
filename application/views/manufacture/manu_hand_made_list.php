...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            ?>
            <?php if($isAdd){ ?>
                <a href="<?=base_url('manu_hand_made/manu_hand_made') ?>" class="btn btn-primary btn-sm pull-right" style="margin: 5px;">Add Manufacture Hand Made</a>
            <?php } ?>
        </h1>
    </section>
    <div class="clearfix">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="box-body table-responsive">
                                        ...
                                        <?php if($this->applib->have_access_role(MANU_HAND_MADE_MODULE_ID,"worker_hisab_handmade")) { ?>
                                            <div class="checked_data" hidden="">
                                                ...
                                                <div class="col-md-2">
                                                    ...
                                                    <input type="text" name="hisab_date" id="datepicker1" class="form-control input-datepicker" value="<?= (isset($mhm_data->mhm_date)) ? date('d-m-Y', strtotime($mhm_data->mhm_date)) : date('d-m-Y'); ?>">
                                                </div>
                                                <div class="col-md-2">
                                        ...
                                        <table id="manu_hand_made_table" class="table row-border table-bordered table-striped" style="width:100%">
                                            <thead>
                                                <tr>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content" >
            <div class="modal-header">
                ...
                <h4 class="modal-title">Manufacture Hand Made Detail Table</h4>
            </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        initAjaxSelect2($("#worker_id"), "<?= base_url('app/worker_supplier_select2_source') ?>");
        initAjaxSelect2($("#operation_id"), "<?= base_url('app/operation_from_department_select2_source/') ?>");
        table = $('#manu_hand_made_table').DataTable({
            "serverSide": true,
            "scrollY": "480px",
            "scrollX": true,
            "search": true,
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('manu_hand_made/manu_hand_made_datatable') ?>",
                "type": "POST",
                "data": function (d) {
        ...
        <?php if($this->applib->have_access_role(MANU_HAND_MADE_MODULE_ID,"worker_hisab_handmade")) { } else { ?>
        ...
        item_table = $('#item_table').DataTable({
            ...
            "paging": false,
            "ajax": {
                "url": "<?php echo site_url('manu_hand_made/manu_hand_made_detail_datatable') ?>",
                "type": "POST",
                "data": function (d) {
                    d.mhm_id = $('#clicked_item_id').val()
                },
            },
            "columnDefs": [
        ...
        $(document).on('click', '.item_row', function () {
            $('#clicked_item_id').val($(this).attr('data-mhm_id'));
            item_table.draw();
            $('#myModal').modal('show');
        });

        $(document).on("click", ".delete_mhm", function () {
            if (confirm('Are you sure delete this records?')) {
                $.ajax({
                    url: $(this).data('href'),
                    type: "POST",
                    data: 'id_name=mhm_id&table_name=manu_hand_made',
                    success: function (response) {
                        var json = $.parseJSON(response);
                        if (json['error'] == 'Error') {
                            show_notify('You cannot delete this Manufacture Hand Made. This Manufacture Hand Made has been used.', false);
                        } else if (json['success'] == 'Deleted') {
                            table.draw();
                            show_notify('Manufacture Hand Made Deleted Successfully!', true);
                        }
                    }
                });
            }
...
//            $("#ajax-loader").show();
        ...
        $(document).on('click', '#save_worker_details', function() {
            ...
            if ($('input.check_mhm:checked').length == 0) {
                show_notify("Please Select Atleast One Item", false);
                return false;
            }
            if ($('input.check_mhm').is(':checked')) {
                $('#ajax-loader').show();
                var checked_items = new Array();
                $.each($("input[name='check_mhm[]']:checked"), function () {
        ...
        $(document).on('change', 'input[name="check_mhm[]"]', function() {
...
//            table.draw();
        ...
        $(document).on('change',"#chk_all",function(){
            ...
            total_receive_fine = 0;
            if(this.checked) {
                $('input[name="check_mhm[]"]').prop('checked', true);
                $('input[name="check_mhm[]"]:checked').change();
            } else {
                $('input[name="check_mhm[]"]').prop('checked', false);
                $('.dataTables_scrollFoot #checked_row_total #checked_row_total_6').html('0.00');
...
//                $('input[name="check_mhm[]"]:not(:checked)').change();
            }
        });
        
        $(document).on("click", ".set_audit_status_yes_no", function () {
            var audit_status = $(this).data('audit_status');
            if(audit_status == '2'){
                var msg = 'Are you sure you want to Set Audit Status to Audited?';
            }
            if(audit_status == '1') {
                var msg = 'Are you sure you want to Set Audit Status to Pending?';
            }
            if (confirm(msg)) {
                $('#ajax-loader').show();
                $.ajax({
