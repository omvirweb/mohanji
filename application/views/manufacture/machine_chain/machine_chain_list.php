...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            ?>
            <?php if($isAdd){ ?>
                <a href="<?=base_url('machine_chain/machine_chain') ?>" class="btn btn-primary btn-sm pull-right" style="margin: 5px;">Add Machine Chain</a>
            <?php } ?>
        </h1>
    </section>
    <div class="clearfix">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="box-body table-responsive">
                                        ...
                                        <div class="col-md-1">
                                            <label for="curb_box">Curb/Box</label>
                                            <select name="curb_box" id="curb_box" class="form-control select2">
                                                <option value=""> - Select - </option>
                                                <option value="<?php echo MCO_SOLDING_CURB_ID; ?>">Curb</option>
                                                <option value="<?php echo MCO_SOLDING_BOX_ID; ?>">Box</option>
                                            </select>
                                        </div>
                                        <div class="col-md-1">
                                            <label for="out_of_curb_box">Out of Inc.</label>
                                            <input type="text" name="out_of_curb_box" id="out_of_curb_box" class="form-control num_only">
                                        </div>
                                        <div class="col-md-1">
                                        ...
                                        <?php if($this->applib->have_access_role(MACHINE_CHAIN_MODULE_ID,"worker_hisab_machine_chain")) { ?>
                                            <div class="checked_data" hidden="">
                                                ...
                                                <div class="col-md-2">
                                                    ...
                                                    <input type="text" name="hisab_date" id="datepicker1" class="form-control input-datepicker" value="<?= (isset($machine_chain_data->machine_chain_date)) ? date('d-m-Y', strtotime($machine_chain_data->machine_chain_date)) : date('d-m-Y'); ?>">
                                                </div>
                                                <div class="col-md-2">
                                        ...
                                        Toggle column: 
                                        <a href="javascript:void(0)" class="toggle-vis department_name_toggle" data-column="1">Department</a> -- 
                                        <a href="javascript:void(0)" class="toggle-vis" data-column="2">Operation</a> -- 
                                        <a href="javascript:void(0)" class="toggle-vis worker_name_toggle" data-column="3">Worker</a> -- 
                                        ...
                                        <table id="machine_chain_table" class="table row-border table-bordered table-striped" style="width:100%">
                                            <thead>
                                                <tr>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content" >
            <div class="modal-header">
                ...
                <h4 class="modal-title">Machine Chain Detail Table</h4>
            </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        initAjaxSelect2($("#operation_id"), "<?= base_url('app/machine_chain_operation_from_department_select2_source/') ?>");
        
        $(document).on('change', '#curb_box', function () {
            var curb_box = $('#curb_box').val();
            if(curb_box != '') {
                $('#ajax-loader').show();
                $.ajax({
                    url: '<?=base_url('app/get_curb_box_default_value')?>',
                    type: "POST",
                    data: 'curb_box='+curb_box,
                    success: function (res) {
                        var json = $.parseJSON(res);
                        $('#out_of_curb_box').val(json['default_value']);
                        $('#ajax-loader').hide();
                    }
                });
            } else {
        ...
        table = $('#machine_chain_table').DataTable({
            "serverSide": true,
            "scrollY": "480px",
            "scrollX": true,
            "search": true,
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('machine_chain/machine_chain_datatable') ?>",
                "type": "POST",
                "data": function (d) {
                    d.department_id = $('#department_id').val();
                    d.worker_id = $('#worker_id').val();
                    d.operation_id = $('#operation_id').val();
                    d.lott_complete = $('#lott_complete').val();
                    d.checked_or_not = $('#checked_or_not').val();
                    d.curb_box = $('#curb_box').val();
                    d.out_of_curb_box = $('#out_of_curb_box').val();
                },
                "complete": function () {
                    $('#ajax-loader').hide();
                },
            ...
            "footerCallback": function ( row, data, start, end, display ) {
                ...
                $(api.column( 8 ).footer()).html(pageTotal8.toFixed(3));
                pageTotal6 = pageTotal8 * 100 / pageTotal7;
                ...
                pageTotal9 = pageTotal11 * 100 / pageTotal10;
                $(api.column( 9 ).footer()).html(pageTotal9.toFixed(2));
        ...
        <?php if($this->applib->have_access_role(MACHINE_CHAIN_MODULE_ID,"worker_hisab_machine_chain")) { } else { ?>
        ...
        item_table = $('#item_table').DataTable({
            ...
            "paging": false,
            "ajax": {
                "url": "<?php echo site_url('machine_chain/machine_chain_detail_datatable') ?>",
                "type": "POST",
                "data": function (d) {
                    d.machine_chain_id = $('#clicked_item_id').val()
                },
            },
            "columnDefs": [
        ...
        $(document).on('click', '.item_row', function () {
            $('#clicked_item_id').val($(this).attr('data-machine_chain_id'));
            item_table.draw();
            $('#myModal').modal('show');
        });

        $(document).on("click", ".delete_machine_chain", function () {
            if (confirm('Are you sure delete this records?')) {
                $.ajax({
                    url: $(this).data('href'),
                    type: "POST",
                    data: 'id_name=machine_chain_id&table_name=machine_chain',
                    success: function (response) {
                        var json = $.parseJSON(response);
                        if (json['error'] == 'Error') {
                            show_notify('You cannot delete this Machine Chain. This Machine Chain has been used.', false);
                        } else if (json['success'] == 'Deleted') {
                            table.draw();
                            show_notify('Machine Chain Deleted Successfully!', true);
                        }
                    }
                });
            }
...
//            $("#ajax-loader").show();
        ...
        $(document).on('click', '#save_worker_details', function() {
            ...
            if ($('input.check_machine_chain:checked').length == 0) {
                show_notify("Please Select Atleast One Item", false);
                return false;
            }
            if ($('input.check_machine_chain').is(':checked')) {
                $('#ajax-loader').show();
                var checked_items = new Array();
                $.each($("input[name='check_machine_chain[]']:checked"), function () {
                    checked_items.push($(this).val());
                });
                var worker_id = $('#worker_id').val();
                var department_id = $('#department_id').val();
                var is_module = '<?php echo HISAB_DONE_IS_MODULE_MC; ?>';
                $.ajax({
        ...
        $(document).on('change', 'input[name="check_machine_chain[]"]', function() {
...
//            table.draw();
        ...
        $(document).on('change',"#chk_all",function(){
            ...
            total_receive_fine = 0;
            if(this.checked) {
                $('input[name="check_machine_chain[]"]').prop('checked', true);
                $('input[name="check_machine_chain[]"]:checked').change();
            } else {
                $('input[name="check_machine_chain[]"]').prop('checked', false);
                $('.dataTables_scrollFoot #checked_row_total #checked_row_total_6').html('0.00');
...
//                $('input[name="check_machine_chain[]"]:not(:checked)').change();
            }
        });
