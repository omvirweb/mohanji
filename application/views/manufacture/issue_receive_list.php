...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            ?>
            <?php if($isAdd){ ?>
                <a href="<?=base_url('manufacture/issue_receive') ?>" class="btn btn-primary btn-sm pull-right" style="margin: 5px;">Add Issue/Receive</a>
            <?php } ?>
        </h1>
    </section>
    <div class="clearfix">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="box-body table-responsive">
                                        ...
                                        <?php if($this->applib->have_access_role(MANUFACTURE_MODULE_ID,"worker_hisab")) { ?>
                                            <div class="checked_data" hidden="">
                                                ...
                                                <div class="col-md-2">
                                                    ...
                                                    <input type="text" name="hisab_date" id="datepicker1" class="form-control input-datepicker" value="<?= (isset($ir_data->ir_date)) ? date('d-m-Y', strtotime($ir_data->ir_date)) : date('d-m-Y'); ?>">
                                                </div>
                                                <div class="col-md-2">
                                        ...
                                        <table id="issue_receive_table" class="table row-border table-bordered table-striped" style="width:100%">
                                            <thead>
                                                <tr>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content" >
            <div class="modal-header">
                ...
                <h4 class="modal-title">Issue/Receive Detail Table</h4>
            </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        table = $('#issue_receive_table').DataTable({
            "serverSide": true,
            "scrollY": "480px",
            "scrollX": true,
            "search": true,
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('manufacture/issue_receive_datatable') ?>",
                "type": "POST",
                "data": function (d) {
        ...
        <?php if($this->applib->have_access_role(MANUFACTURE_MODULE_ID,"worker_hisab")) { } else { ?>
        ...
        item_table = $('#item_table').DataTable({
            ...
            "paging": false,
            "ajax": {
                "url": "<?php echo site_url('manufacture/issue_receive_detail_datatable') ?>",
                "type": "POST",
                "data": function (d) {
                    d.ir_id = $('#clicked_item_id').val()
                },
            },
            "columnDefs": [
        ...
        $(document).on('click', '.item_row', function () {
            $('#clicked_item_id').val($(this).attr('data-ir_id'));
            item_table.draw();
            $('#myModal').modal('show');
        });

        $(document).on("click", ".delete_ir", function () {
            if (confirm('Are you sure delete this records?')) {
                $.ajax({
                    url: $(this).data('href'),
                    type: "POST",
                    data: 'id_name=ir_id&table_name=issue_receive',
                    success: function (response) {
                        var json = $.parseJSON(response);
                        if (json['error'] == 'Error') {
                            show_notify('You cannot delete this Issue/Receive. This Issue/Receive has been used.', false);
                        } else if (json['success'] == 'Deleted') {
                            table.draw();
                            show_notify('Issue/Receive Deleted Successfully!', true);
                        }
                    }
                });
            }
...
//            $("#ajax-loader").show();
        ...
        $(document).on('click', '#save_worker_details', function() {
            ...
            if ($('input.check_ir:checked').length == 0) {
                show_notify("Please Select Atleast One Item", false);
                return false;
            }
            if ($('input.check_ir').is(':checked')) {
                $('#ajax-loader').show();
                var checked_items = new Array();
                $.each($("input[name='check_ir[]']:checked"), function () {
        ...
        $(document).on('change', 'input[name="check_ir[]"]', function() {
...
//            table.draw();
        ...
        $(document).on('change',"#chk_all",function(){
            ...
            total_receive_fine = 0;
            if(this.checked) {
                $('input[name="check_ir[]"]').prop('checked', true);
                $('input[name="check_ir[]"]:checked').change();
            } else {
                $('input[name="check_ir[]"]').prop('checked', false);
                $('.dataTables_scrollFoot #checked_row_total #checked_row_total_5').html('0.00');
...
//                $('input[name="check_ir[]"]:not(:checked)').change();
            }
        });
